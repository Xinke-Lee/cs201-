...
├── dataset/                    # 数据集目录
├── outputs/                    # 推理结果、曲线图、预览图输出目录，每一个原始视频输出一个追踪效果视频、位移时间曲线csv、并绘制成图
...
这里我们已经有标注好了的训练集和验证集了，如果想更新，使用网页工具重新标注就好了。
...
这里我跑了5个，呈现很清晰的阻尼振动，追踪效果可以见视频（视频有点长，只能下载下来再看）。
...
## 4.总结
尽管完成这个大作业后，回头看来，用YOLO处理标记追踪有点大炮打蚊子的感觉。但是我仍然学习了用YOLO解决问题的流程：训练、验证、使用。YOLO在实际场景中的使用相当广泛，用这个最简单单任务来对YOLO建立一个初步的了解，从过程上是0到1的巨大突破。
